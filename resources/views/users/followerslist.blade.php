                    ...
                    <ul class="list-group">
                        @foreach ($users as $user)
                            <li class="list-group-item d-flex justify-content-between align-items-center text-primary">
                                <img class="float-right w-auto h-auto" src="{{ asset('images/'. $user->followers()->avatar) }}" style="max-height:80px" alt="PHOTO">
                                <a href="{{ route('users.show',['id' => $user->id]) }}" class="btn btn-praimary">{{ $user->followers()->first_name }} {{ $user->followers()->last_name }}</a>
